...
void ElasticsearchClient::setIndexMapping(const char* indexName, const JsonDocument& mapping)
{
  String payload;
...
void ElasticsearchClient::setIndexIlmPolicy(const char* indexName, const JsonDocument& ilmPolicy)
{
  String payload;
...
void ElasticsearchClient::uploadData(const char* indexName, const JsonDocument& data, const char* indexId)
{
  String payload;
  serializeJson(data, payload);

  m_http.begin(m_host, m_port, String(indexName) + "/_doc/" + indexId + "?pretty");
  m_http.addHeader("Content-Type", "application/json");
